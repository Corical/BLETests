    ...
    subscription = reactiveBeacons.observe()
        ...
        .subscribe(new Action1<Beacon>() {
          @Override public void call(Beacon beacon) {
              String beaconIdentifier = beacon.device.getAddress();
              if (beacon.rssi > -80)
            {
                Log.i("AppText", "AppText - Discovered beacon: " + beaconIdentifier + " (RSSI: " + beacon.rssi + ")");

                if (beaconExists(beaconIdentifier))
                {
                    ScheduledFuture<?> scheduledInstance = timers.get(beaconIdentifier);
                    boolean cancelResult = scheduledInstance.cancel(true);
                    Log.i("AppText", "AppText - Attempted to canceled runnable, cancel successful: " + cancelResult);
                    Object value = timers.remove(scheduledInstance);
                    Log.i("AppText", "AppText - Attempted to remove scheduledInstance, removal successful: " + (value == null));
                }

                UnsubscribeTimer timerRunnable = new UnsubscribeTimer(beaconIdentifier);
                ScheduledFuture<?> scheduledInstance = new Scheduler().GetScheduledInstance(timerRunnable);
                timers.put(beaconIdentifier, scheduledInstance);
                beacons.put(beaconIdentifier, beacon);
                refreshBeaconList();
            }
              else
            {
                Log.i("AppText", "AppText - Discovered out of range beacon: " + beaconIdentifier + " (RSSI: " + beacon.rssi + ")");
            }
          }
        });
  }

    private boolean beaconExists(String identifier)
    {
